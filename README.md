...
这个问题暴露了当前 agent 不加审视就继承的六个设计选择：**亏损的路由、挤占上下文的工具调用、盲目压缩的 compaction、几乎不触发的子 agent、丢弃状态的重启、以及内置能力之争。**
...
| 6. 内置能力之争 | 每项内置能力都永久消耗上下文 | 在易用性与专业用户需求之间被迫二选一 |
...
关于 agent 是否该内置能力，有一场持续的争论。今天的取舍落在「易用性」（功能高度内置的 agent 在一端）和「专业用户需求」（Claude Code、Codex 这类面向专业用户的工具在另一端）之间。

**这个取舍存在，仅仅因为每项内置能力都永久消耗上下文。把这个成本拿掉，争论就消解了。**
...
它们背后是「在需要时展开可用动作的完整 schema」的能力，类似工具搜索工具。

**把这一切绑在一起的要求是：这些东西在不再需要之后，都不能污染上下文。**

![图 5：三级披露](figures/fig5.png)

*图 5. 三级披露。模型看到一张廉价的全局地图，只为选中的内容承担详细信息的上下文开销，用完之后把细节从上下文中丢掉。*

**如果这能成，第二节里的内置能力之争就消失了：当一项内置能力在被使用之前几乎不花成本，agent 就能带着几百个工具和几千页文档发布。**
...
改前端代码就加载风格指南；在某个子目录里工作就加载那个目录的避坑说明（**而且笔记建议每个子目录都该有一份**）。
...
笔记在日常对话里观察到同样的需求：请求摘要应该拉入偏好的格式；请求用自己风格写作应该拉入样本和一份「不要什么」的清单；请求代码应该拉入风格指南和一条「不要加一百万条断言」的提示。

### A. 结构化 skills

harness 的可编程程度越高，skills 就越能携带行为变更，而不只是指令，类似 Claude Code 的 skill hooks，但能力更强。
...
**若将状态保存在具名变量中，系统可能比「状态就是上下文窗口里恰好剩下的东西」更清晰。**
...
*图 7. 显式状态上的后台处理。找出与某个改动相关的信息，这项昂贵工作只做一次，再由每个只读后台任务共享。*
...
把读写显式区分开，用笔记的话说，**最终应该显著增强 agent 的能力。**
...
- **子 agent 稀少**是因为传状态很难
- **重启**把好状态和坏状态一起丢掉
- **内置能力之争**之所以存在，只是因为每项内置能力永久消耗上下文
