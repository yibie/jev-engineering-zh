...
- 关于 agent 是否该内置「电池」（内置能力），有很大的争论
- 例子：https://x.com/sudoingx/status/2061073944611029393
- 往往存在一个取舍：易用性（openclaw 是极端）对高级用户（比如 claude code / codex）
...
- 对任何一个用户查询，我们都可以重新计算：
  - **先前的上下文有多好**
    - 也即：复用现有 KV cache 是自然合理的，但我们应该基于「复用当前 cache 有多好 vs 从头重建」做有依据的、成本感知的决策
...
- 这很可能也会让我们向用户暴露更多旋钮：多花钱换更好更快，还是保守地尽量少花

**从第一性原理出发的 Skills / MCP / 工具调用**

- 我认为中间应该存在一样东西：
  - **关于「有什么可用」的小片段**
    - 因为模型怎么可能提出一个它连方向都不知道存在的动作
    - 类似 skills 的描述
      - 但它不必放在系统消息里，可以在合适时动态加载
  - **在需要时倾倒出可用动作的完整 schema**
    - 类似 tool search tool
  - **而且这不污染上下文**
    - 前所未有 (:
- 如果这能成，我们就能真正带上各种各样的「电池」而成本几乎为零
  - 如果能以近乎零成本内置东西，我们会拿到一个了不起的超能力——无论是联合营销，还是「让事情就是能用™」
  - （想象一下内建了几百个工具加几千页文档）
...
- 我们可以按不同条件动态加载 AGENTS.md 的不同部分
  - 比如：
    - 是前端吗？加载风格指南
    - 在这个子目录里吗？加载坑 / 陷阱文件（说实话，每个子目录都该有个陷阱文件）
...
- 大意是更多变量 / 显式状态
- **存在一个世界，那里把状态当作显式变量会干净得多**

**更花哨的摘要**

- 一个想法是：如果我们能把「这段 grep 输出里哪部分相关」做成热力图，我们就能按喜欢的任意程度动态过滤它
- **它会看起来很酷**

**极端子 agent + 并行化**

- 不确定这对我们会怎么运作，但（因为上面那节）同时派发很多任务可能变得非常容易，然后我们就得解决那些烦人但有用的东西，比如同步原语、甚至 agent 间通信
...
- 进一步延伸：除了难度 / 成本，可能还有很多其他理由去路由（比如：不要用 anthropic 的模型做 LLM 研究，不要用 openai / anthropic 做安全敏感的事）
...
- 我对一个以 TypeSafe 为中心的 coding agent 的假设之一是：**我们必须对状态（上下文里的一切）非常显式**
  - 具体来说，通过对状态保持聪明，我们能在路由、子 agent 等方面高效得多
- 这一点与只读后台任务之间的协同看起来非常高！
  - 找出与某个代码改动相关的信息，想必是件不平凡的工作
  - **如果这份工作能在所有后台任务间共享**，运行它们就会便宜得多，也可能经济到能跑比原来多得多的数量

🙃

- 我认为对状态保持显式（区分读与写）最终应该会给我们超能力
